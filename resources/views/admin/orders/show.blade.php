...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            ...
            <div class="bg-white shadow rounded-lg overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
                    <h2 class="text-lg font-medium">Детали заказа</h2>
                    <span class="px-3 py-1 rounded-full text-xs font-medium 
                        ...
                        {{ $order->status === 'completed' ? 'Завершён' : 
                          ...
                          ($order->status === 'cancelled' ? 'Отменён' : 'Ожидание')) }}
                    </span>
                </div>
                <div class="px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <h3 class="text-sm font-medium text-gray-500">Информация о клиенте</h3>
                    ...
                    <div>
                        <h3 class="text-sm font-medium text-gray-500">Информация о доставке</h3>
                        <div class="mt-2 space-y-1">
                            ...
                            <p class="text-sm flex items-center">
                                ...
                                {{ $order->shipping_method ?? 'Стандартная доставка' }}
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <!-- Карточка товаров заказа -->
            <div class="bg-white shadow rounded-lg overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h2 class="text-lg font-medium">Товары в заказе</h2>
                </div>
                <div class="divide-y divide-gray-200">
                    ...
                    <div class="px-6 py-4 flex">
                        ...
                        <div class="ml-4 flex-1">
                            ...
                            <div class="mt-1 text-sm text-gray-500">Артикул: {{ $item->product->sku ?? 'Не указан' }}</div>
                            <div class="mt-1 text-sm text-gray-500">Количество: {{ $item->quantity }}</div>
                            <div class="mt-1 text-sm font-medium text-gray-900">Итого: {{ number_format($item->price * $item->quantity, 2) }} ₽</div>
                        </div>
                    </div>
                    @endforeach
                </div>
                <div class="px-6 py-4 border-t border-gray-200">
                    <div class="flex justify-between">
                        <div class="text-sm text-gray-500">Подытог</div>
                        <div class="text-sm font-medium">{{ number_format($order->subtotal, 2) }} ₽</div>
                    </div>
                    <div class="flex justify-between mt-2">
                        <div class="text-sm text-gray-500">Доставка</div>
                        <div class="text-sm font-medium">{{ number_format($order->shipping, 2) }} ₽</div>
                    </div>
                    @if($order->discount > 0)
                    <div class="flex justify-between mt-2">
                        <div class="text-sm text-gray-500">Скидка</div>
                        <div class="text-sm font-medium text-green-600">-{{ number_format($order->discount, 2) }} ₽</div>
                    </div>
                    @endif
                    <div class="flex justify-between mt-4 pt-4 border-t border-gray-200">
                        <div class="text-base font-medium">Общая сумма</div>
                        <div class="text-base font-bold">{{ number_format($order->total, 2) }} ₽</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white shadow rounded-lg overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h2 class="text-lg font-medium">Сводка заказа</h2>
                </div>
                <div class="px-6 py-4 space-y-4">
                    <div>
                        <h3 class="text-sm font-medium text-gray-500">Способ оплаты</h3>
                        <p class="mt-1 text-sm flex items-center">
                            ...
                            {{ $order->payment_method === 'paypal' ? 'PayPal' : 'Наличные' }}
                            @endif
                        </p>
                    </div>
                    <div>
                        <h3 class="text-sm font-medium text-gray-500">Дата заказа</h3>
                        <p class="mt-1 text-sm">{{ $order->created_at->format('d.m.Y в H:i') }}</p>
                    </div>
                    @if($order->notes)
                    <div>
                        <h3 class="text-sm font-medium text-gray-500">Комментарии клиента</h3>
            ...
            <!-- Карточка изменения статуса -->
            <div class="bg-white shadow rounded-lg overflow-hidden">
                <div class="px-6 py-4 border-b border-gray-200">
                    <h2 class="text-lg font-medium">Изменение статуса</h2>
                </div>
                <div class="px-6 py-4">
