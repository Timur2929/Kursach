    ...
    <div class="bg-white shadow-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between">
                <div>
                    <h1 class="text-3xl font-bold text-gray-900">Наша Коллекция</h1>
                    
                    <p class="mt-2 text-gray-600">Откройте для себя наши handmade украшения</p>
                </div>
                
                <!-- Сортировка -->
                <div class="mt-4 md:mt-0">
                    <form method="GET" action="{{ route('shop') }}">
                        <label for="sort" class="sr-only">Сортировать по</label>
                        <select id="sort" name="sort" onchange="this.form.submit()" class="block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-emerald-500 focus:border-emerald-500 sm:text-sm rounded-md">
                            ...
                            <option value="popular" {{ request('sort') == 'popular' ? 'selected' : '' }}>Самые популярные</option>
                        </select>
                    </form>
                </div>
            </div>
            ...
            <div class="mt-4">
                <div class="flex flex-wrap gap-2 items-center">
                    ...
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">
                        Категория: {{ \App\Models\Category::find(request('category'))->nom ?? '' }}
                    ...
                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">
                        ...
                        @endphp
                        Цена: {{ $priceRanges[request('price')] ?? '' }}
                    ...
                    <a href="{{ route('shop') }}" class="text-sm text-emerald-600 hover:text-emerald-800">Сбросить все</a>
                </div>
            </div>
            @endif
        </div>
            ...
            <div class="md:w-64 pr-8 mb-8 md:mb-0">
                <form method="GET" action="{{ route('shop') }}">
                    <div class="bg-white p-4 rounded-lg shadow-sm sticky top-4">
                        ...
                        <div class="mb-6">
                            <h3 class="text-lg font-medium text-gray-900 mb-3">Категории</h3>
                            <ul class="space-y-2">
                        ...
                        <!-- Фильтр по цене -->
                        <div class="mb-6">
                            <h3 class="text-lg font-medium text-gray-900 mb-3">Цена</h3>
                            <div class="space-y-4">
            ...
            <div class="flex-1">
                        ...
                        <div class="group relative bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-md transition-shadow duration-300 border border-gray-100">
                            ...
                            <div class="absolute top-2 left-2 z-10 space-y-1">
                                ...
                                <span class="block bg-red-500 text-white text-xs font-bold px-2 py-1 rounded-full">
                                    АКЦИЯ -{{ $product->discount_percent }}%
                                </span>
                                @endif
                                    ...
                                    <div>
                                        @if($product->on_sale)
                                            <span class="text-lg font-bold text-gray-900">
                                                {{ number_format($product->sale_price, 2) }} Руб
                                            </span>
                ...
                @else
                    <div class="text-center py-12">
                        ...
                        <h3 class="mt-2 text-lg font-medium text-gray-900">Товары не найдены</h3>
