    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
                ...
                <ol class="flex items-center space-x-2">
                    <li>
                        <div class="flex items-center">
                            <a href="{{ route('home') }}" class="text-emerald-600 hover:text-emerald-800">Главная</a>
                        </div>
                    </li>
                    <li>
                        <div class="flex items-center">
                            ...
                            <a href="{{ route('shop') }}" class="ml-2 text-emerald-600 hover:text-emerald-800">Магазин</a>
                        </div>
                    </li>
                    @if($product->category)
                    <li>
            ...
            <div class="md:flex">
                ...
                <div class="md:w-1/2 p-6">
                    ...
                    <div class="flex justify-between items-start">
                        ...
                        <!-- Кнопка избранного -->
                        <button class="text-gray-400 hover:text-red-500 transition-colors" title="Добавить в избранное">
                    ...
                    <div class="mt-4 flex items-center">
                        <div class="flex items-center">
                            ...
                            @endfor
                            <span class="ml-2 text-gray-600">{{ number_format($average, 1) }}/5 ({{ $count }} отзывов)</span>
                        </div>
                        <a href="#reviews" class="ml-4 text-sm text-emerald-600 hover:text-emerald-800">Смотреть отзывы</a>
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <div class="flex items-center">
                            ...
                            <span class="ml-3 text-xl text-gray-500 line-through">{{ number_format($product->prix, 2) }} ₽</span>
                            <span class="ml-3 px-2 py-1 bg-red-100 text-red-800 text-sm font-bold rounded-full">
                                -{{ $product->discount_percent }}%
                            </span>
                        </div>
                        @else
                        <span class="text-3xl font-bold text-gray-900">{{ number_format($product->prix, 2) }} ₽</span>
                        @endif
                    </div>
                    ...
                    <form action="{{ route('cart.add', $product->id) }}" method="POST" class="mt-6">
                        @csrf
                        <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-6 py-3 rounded-md font-medium transition-colors">
                            Добавить в корзину
                        </button>
                    </form>
                    ...
                    <div class="mt-8">
                        ...
                        <div class="prose max-w-none text-gray-600">
                            {!! $product->description ?? '<p class="text-gray-500">Описание отсутствует</p>' !!}
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="mt-12 bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-2xl font-bold text-gray-900">Оставить отзыв</h2>
            </div>
                ...
                @auth
                    <form action="{{ route('reviews.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="note" required class="w-full px-3 py-2 border border-gray-300 rounded-md">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="commentaire" required class="w-full px-3 py-2 border border-gray-300 rounded-md" rows="3"></textarea>
                        </div>
                        
                        <button type="submit" class="bg-emerald-600 hover:bg-emerald-700 text-white px-4 py-2 rounded-md font-medium">
                            Отправить отзыв
                        </button>
                    </form>
                @else
                    <div class="text-center py-4">
                        <p class="text-gray-600">Чтобы оставить отзыв, необходимо авторизоваться.</p>
                        <div class="mt-4">
                            <a href="{{ route('login') }}" class="text-emerald-600 hover:text-emerald-800 font-medium">Войти</a>
                            <span class="mx-2 text-gray-500">или</span>
                            <a href="{{ route('register') }}" class="text-emerald-600 hover:text-emerald-800 font-medium">Зарегистрироваться</a>
                        </div>
                    </div>
                @endauth
            </div>
        ...
        <div id="reviews" class="mt-12 bg-white rounded-lg shadow overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-200">
                <h2 class="text-2xl font-bold text-gray-900">Отзывы покупателей</h2>
            </div>
            ...
            <div class="p-6">
                @if($product->reviews && $product->reviews->count() > 0)
                    ...
                    <div class="mb-6 pb-6 border-b border-gray-200 last:border-0 last:mb-0 last:pb-0">
                        ...
                        <p class="text-sm text-gray-500 mt-2">- {{ $review->user->name ?? 'Аноним' }}</p>
                    </div>
                    @endforeach
                @else
                    <div class="text-center py-8">
                        ...
                        <h3 class="mt-2 text-lg font-medium text-gray-900">Нет отзывов об этом товаре</h3>
                        <p class="mt-1 text-gray-500">Будьте первым, кто оставит отзыв.</p>
                    </div>
                @endif
            </div>
        </div>
        ...
        <div class="mt-12">
            <h2 class="text-2xl font-bold text-gray-900 mb-6">Рекомендуем также</h2>
                            ...
                            <div>
                                @if($relatedProduct->on_sale)
                                    ...
                                    <span class="text-sm text-gray-500 line-through ml-2">
                                        {{ number_format($relatedProduct->prix, 2) }} ₽
                                    </span>
                                @else
                                    <span class="text-lg font-bold text-gray-900">
                                        {{ number_format($relatedProduct->prix, 2) }} ₽
...
<script>
    ...
    // Инициализация рейтинга, если было старое значение
